...
# only allow certain events into the queue to reduce lag
...
# shid and fard, pygame rects can only store ints so i needta have another set of attributes for more accurate movement
...
class Ball:
	...
	def change_grav_magnitude(self, increase=True):
		if increase:
			if self.grav_magnitude < self.MAX_GRAVITY: self.grav_magnitude += self.GRAVITY_INC
	...
	def on_wall_collision(self, vel_index, clones):			# ...just how many methods am i even making to avoid copypasting code
		if abs(self.velocity[vel_index]) > 4 * abs(self.grav_magnitude):
			...
			if clones: self.spawn_clone(self.img, (1,2), 0.5, FRAMERATE*2)
		else:
	...
	while game_active:
		for event in pygame.event.get():
			if event.type == pygame.KEYDOWN:
				...
				except TypeError: gameloop_key_funcs[event.key](event.key)
			elif event.type == pygame.QUIT: adios()
