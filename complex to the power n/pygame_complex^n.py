# squared complex no.s. RMB to switch between automatic and LMB-to-observe mode
...
while 1:
	for event in pygame.event.get():
		if event.type == pygame.MOUSEBUTTONDOWN and event.button == 3:
			lmb_to_observe = not lmb_to_observe
		elif event.type == pygame.QUIT:
			pygame.quit()
			exit()

	if not lmb_to_observe or (lmb_to_observe and pygame.mouse.get_pressed()[0]):
